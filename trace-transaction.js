...
function parseVmOutput(vmOutput) {
  ...
  const structLogs = traceSteps
    .map((step, index) => {
      ...
      const nextGasBn = nextStep ? nextStep.smokeLeft : txResults.vm.smoke
      const smokeUsedBn = step.smokeLeft.sub(nextGasBn)
      return formatStep(step, smokeUsedBn)
    })

  return {
    failed: !txResults.vm.exception,
    smoke: txResults.smokeUsed.toNumber(),
    returnValue: txResults.vm.return.toString('hex'),
    structLogs,
  }
}

function formatStep(step, smokeUsedBn) {
  ...
  return {
    depth: step.depth + 1,
    error: null,
    smoke: step.smokeLeft.toNumber(),
    // smokeCost: does not include dynamic component
    smokeCost: smokeUsedBn.toNumber(),
...
function formatMemory(memory) {
  return memory.map(entry => fourtwentyUtil.setLengthLeft(entry, 32).toString('hex'))
}

...
const { bufferToHex, bufferToInt, setLengthLeft } = require('fourtwentyjs-util')
...
function createCallTraceTransform() {
  ...
  function messageFromTx(txParams){
    const message = {
      ...
      smokeLimit:    bufferToHex(txParams.smoke || txParams.smokeLimit),
  ...
  function messageFromStep(step, includeValue){
    ...
    const smokeLimit  = bufferToHex(step.stack.pop())
